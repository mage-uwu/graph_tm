...
EDITS = [
    ...
    ("    float peak_lr,sharp,clip,mask_prob,scale,mlm_scale;\n} Config;",
     "    float peak_lr,sharp,clip,mask_prob,scale,mlm_scale;\n    int gevery,gch;  /* global view: every gevery-th block input gets gch OR-pooled channels */\n} Config;"),
    ...
    ("typedef struct { size_t n; float *z,*g,*m,*v; uint64_t t; } Param;",
     ...
     "static int in_ch(const Config *c,int i){return (i?c->width:c->bits)+(gblock(c,i)?c->gch:0);}\n"
     "typedef struct { size_t n; float *z,*g,*m,*v; uint64_t t; } Param;"),
    ...
    ("static void encode(Net *n,Work *w) {",
     ...
     "        for(int t=0;t<T;t++)if(w->valid[(size_t)b*T+t]){float v=x[((size_t)t*W+c)*B+b];if(at<0||v>m){m=v;at=t;}}\n"
     "        am[(size_t)c*B+b]=at;\n"
     ...
     "    #pragma omp parallel for schedule(static)\n"
     "    for(int c=0;c<G;c++)for(int b=0;b<B;b++) {\n"
     "        int32_t at=am[(size_t)c*B+b];if(at<0)continue;\n"
     "        float s=0.f;for(int t=0;t<T;t++)s+=dx[((size_t)t*CA+W+c)*B+b];\n"
     "        o[((size_t)at*W+c)*B+b]+=s;\n"
     "    }\n"
    ...
    ('    char *tmp=temp_path(path);FILE *f=open_file(tmp,"wb");write_bytes(f,"LTCKP001",8);config_write(f,n->c);',
     '    char *tmp=temp_path(path);FILE *f=open_file(tmp,"wb");write_bytes(f,n->c.gevery?"LTCKP002":"LTCKP001",8);config_write(f,n->c);\n'
     '    if(n->c.gevery){put32(f,(uint32_t)n->c.gevery);put32(f,(uint32_t)n->c.gch);}'),
    ('    if(memcmp(magic,"LTCKP001",8))die("%s is not a training .ltc checkpoint",path);\n    Config c=config_read(f);Net *n=net_new(c,1,0,1);',
     ...
     '    Config c=config_read(f);if(v2){c.gevery=(int)get32(f);c.gch=(int)get32(f);validate(c);}Net *n=net_new(c,1,0,1);'),
    ('    char *tmp=temp_path(path);FILE *f=open_file(tmp,"wb");write_bytes(f,"LTHRD001",8);',
     '    char *tmp=temp_path(path);FILE *f=open_file(tmp,"wb");write_bytes(f,h->c.gevery?"LTHRD002":"LTHRD001",8);'),
    ("    for(int i=0;i<10;i++)put32(f,a[i]);",
     "    for(int i=0;i<10;i++)put32(f,a[i]);\n    if(c.gevery){put32(f,(uint32_t)c.gevery);put32(f,(uint32_t)c.gch);}"),
    ...
    ("    for(int i=0;i<10;i++){uint32_t v=get32(f);if(v>INT_MAX)die(\"invalid hard config\");*a[i]=(int)v;}validate(c);h->c=c;",
     ...
     "    if(hv2){c.gevery=(int)get32(f);c.gch=(int)get32(f);}validate(c);h->c=c;"),
    ...
    ("        hard_layer(h->l+i,cur,next,w->mask,w->P,T);uint64_t *tmp=cur;cur=next;next=tmp;",
     ...
     "                for(int c=0;c<G;c++) {\n"
     "                    uint64_t any=0;for(int t=0;t<T;t++)any|=cur[((size_t)p*T+t)*W+c];\n"
    ...
    ("    validate(*c);\n}\n#ifndef LOGIC_NO_MAIN",
     "    if(o->changed&(UINT64_C(3)<<21)) {\n"
     "        if(c->gevery!=s->gevery || c->gch!=s->gch)die(\"--global-every/--global-channels differ from the checkpoint\");\n"
     "    }\n"
     "    validate(*c);\n}\n#ifndef LOGIC_NO_MAIN"),
]
