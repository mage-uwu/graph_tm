 ...
 *   speed      training: deterministic parallel backward (phase 1 over outputs, phase 2 over input channels in
 *              the serial order), ~0.4 s/step (16x1024, seq 64, batch 32, 8 cores). The binary sets
 *              OMP_WAIT_POLICY=passive itself: spinning OpenMP threads cost 5-10x on SMT machines.
